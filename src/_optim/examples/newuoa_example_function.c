...
static double function(long n, const double *x) {
    ...
    return -4*x[0]*x[1] + 5*x[0]*x[0] + 8*x[1]*x[1] + 16*sqrt(5.)*x[0] + 8*sqrt(5.)*x[1] - 44.;
}
...
//    const int number_of_interpolation_conditions = variables_count + 2;
    ...
    double variables_values[] = {0., -sqrt(5.)};
    ...
    const size_t working_space_size = NEWUOA_WORKING_SPACE_SIZE(variables_count, number_of_interpolation_conditions);
