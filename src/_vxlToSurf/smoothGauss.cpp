...
int  smoothSurf(InputFile& inp, facePieceList& facezsZ, piece<point> pointsAll)  {
    ...
    ensure(relaxCL<=0.00000001,"relaxCL can lead to inaccurate results",0); //
     ...
     for (int ifcs=facezsZ.size()-1; ifcs>=0; --ifcs) if(facezsZ[ifcs].size()) {
        ...
        vectorField fNorms(facezs.size());
      {
        ...
        if(iter==nIters-1) {
            ...
            dbls  pmrks(pInd+1);   for_(pValidIs,i) if(pValidIs[i]!=-1) { pmrks[pValidIs[i]]=pMarks[i]; }
            vtk<< "\npmrks 1 " << pmrks.size() << " int\n" ;
            for_(pmrks,ii) vtk<<pmrks[ii]<<'\n';

            dbl3s dAvgs(pInd+1);   for_(pValidIs,i) if(pValidIs[i]!=-1) { dAvgs[pValidIs[i]]=dispAvg[i]; }
            vtk<< "\ndAvgs 3 " << dAvgs.size() << " float\n" ;
            for_(dAvgs,ii) vtk<<dAvgs[ii]<<'\n';

            dbl3s dAvgO(pInd+1);   for_(pValidIs,i) if(pValidIs[i]!=-1) { dAvgO[pValidIs[i]]=dispAvgO[i]; }
